...
if (isset($_REQUEST["edit_id"])) {
        ...
        $send_to_area = $_REQUEST["send_to_area"];
        $send_refill = $_REQUEST["send_refill"];

        $qry = $db->query("UPDATE `tank_entry_master` SET `tank_entry_date`='$date',`area_id`='$select_area',`opening_meter`='$opening_meter',`total_refil`='$total_refill',`closing_meter`='$closing_meter',`diesel_out`='$desel_out',`tank_balance`='$balance',`send_area_id`= $send_to_area,`send_refill`=' $send_refill',`updated_by`=' $created_by', `updated_on`=NOW() WHERE id= $edit_id") or die("");
        ...
        $send_to_area = $_REQUEST["send_to_area"];
        $send_refill = $_REQUEST["send_refill"];
        $qry = $db->query("INSERT INTO `tank_entry_master`(`tank_entry_date`, `area_id`, `opening_meter`, `total_refil`, `closing_meter`, `diesel_out`, `tank_balance`,`send_area_id`,`send_refill`,`created_on`, `updated_by`) VALUES ('$date','$select_area','$opening_meter','$total_refill','$closing_meter','$desel_out','$balance',$send_to_area ,'$send_refill',NOW(),'$created_by')") or die("");
        ...
        if ($area_id != 0) {
                ...
                if ($row || $row2) {
                        ...
                        if ($one > $two) {
                                ...
                                if ($row) {
                                        $closing_meter = $row['closing_meter'] + $row2['send_refill'];
                                } else {
                                        $closing_meter = $row2['send_refill'];
                                }
                                ?>
                                                        <input type="hidden" id="tank_balance" value="<?= $row2['send_refill'] ?>">
                                                        <input type="hidden" id="tank_opening" value="<?= $closing_meter ?>">
                                <?php
                        }
                } else {
